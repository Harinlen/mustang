...
| CLI Phase D planning | Added `docs/plans/cli-phase-d-session-config-theme.md`, splitting Phase D out of `cli-plan.md` into a detailed implementation plan for ACP-backed session selection/resume, `~/.mustang/client.yaml` local config, theme selection, startup orchestration, optional local kernel autostart, and the Phase D test/probe matrix. | planning only; no CLI runtime code changed |
